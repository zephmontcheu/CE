...
double par_norm_residual1 ( double const *u , size_t const n){
	double sum = 0;
        size_t n_t = n+2;
    for (int i =1 ; i <= n ; i++){
...
int par_jacobi(double *u, double *u_new, size_t const n, size_t const steps, int threads) {
	size_t n_t = n +2;
	 int real_steps = 0;
	
	 for (int k = 0 ; k < steps ; k++){
	 	if(par_norm_residual1(u,n) < 0.1){
   			printf("steps = %d \n",real_steps);
   			return real_steps;
   			}
   		real_steps++;
   	 	...
   	 	for ( int i = 1; i <=n ; i++ ){
   			for (int j = 1 ; j <= n ; j++){
   				if( i * n_t +j == n_t * n_t /2)
   		 		...
   		 		u_new[ i * n_t + j]= ( u[(i +1) * n_t +j] + u[(i-1) * n_t + j]
   		 		 +u[ i * n_t +(j+1)]   + u[ i * n_t +(j-1)])/4;
   			}
   		}
...
int main(int argc, char **argv) {
    ...
    int k = par_jacobi(u, u_new, n, iterations,num_threads); 
    ...
    elapsed_time += (end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec) * 1e-9;
    printf(" time = %f \n " , elapsed_time);
